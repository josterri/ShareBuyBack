...
def analyze_dca_strategy(price_paths: np.ndarray, daily_investment: float) -> pd.DataFrame:
    ...
    performance_bps = (1 - (avg_execution_price / simple_avg_price)) * 10000
...
def analyze_adaptive_strategy(price_paths: np.ndarray, total_investment: float, base_n_days: int, min_investment_days: int, max_investment_days: int, modulation_factor: float) -> pd.DataFrame:
    ...
    for i in range(n_paths):
        ...
        for t in range(1, len(path)):
            # Hard stop if max duration is reached
            if days_invested >= max_investment_days:
                break
            
            # Stop if budget is exhausted AND minimum investment period is met
            if remaining_investment <= 1e-6 and days_invested >= min_investment_days:
                break
            ...
            modulation = modulation_factor if current_price < running_avg else 1 / modulation_factor
            target_investment = base_daily_investment * modulation
            
            actual_investment = min(target_investment, remaining_investment)
            
            shares_bought = actual_investment / current_price if current_price > 0 else 0
        ...
        # Benchmark is the simple average over the actual investment period
        simple_avg_price = np.mean(path[1:days_invested+1]) if days_invested > 0 else 0
        performance_bps = (1 - (avg_execution_price / simple_avg_price)) * 10000 if simple_avg_price > 0 and avg_execution_price > 0 else 0
        
        results.append({
            'path_id': i + 1,
            'terminal_price': path[-1],
            'total_usd_invested': total_cost,
            'total_shares_acquired': total_shares,
            'avg_execution_price': avg_execution_price,
            'simple_avg_price': simple_avg_price,
            'performance_bps': performance_bps,
...
def analyze_dynamic_adaptive_strategy(price_paths: np.ndarray, total_investment: float, min_investment_days: int, max_investment_days: int) -> pd.DataFrame:
    ...
    for i in range(n_paths):
        ...
        for t in range(1, len(path)):
            # Hard stop if max duration is reached or budget is spent
            if days_invested >= max_investment_days or remaining_investment <= 1e-6:
                break
            ...
            target_duration = min_investment_days if current_price < running_avg else max_investment_days
            
            remaining_days_to_target = target_duration - days_invested
            
            if remaining_days_to_target <= 0:
                target_investment = remaining_investment
            else:
                target_investment = remaining_investment / remaining_days_to_target
        ...
        avg_execution_price = total_cost / total_shares if total_shares > 0 else 0
        # Correctly benchmark only against the prices during the actual investment period
        simple_avg_price = np.mean(path[1:days_invested+1]) if days_invested > 0 else 0
        performance_bps = (1 - (avg_execution_price / simple_avg_price)) * 10000 if simple_avg_price > 0 and avg_execution_price > 0 else 0
...
def analyze_vol_scaled_adaptive_strategy(price_paths: np.ndarray, total_investment: float, min_investment_days: int, max_investment_days: int, sigma: float) -> pd.DataFrame:
    ...
    for i in range(n_paths):
        ...
        for t in range(1, len(path)):
            if days_invested >= max_investment_days or (remaining_investment <= 1e-6 and days_invested >= min_investment_days):
                break
            ...
            if len(hist_prices) < 2:
                ...
                running_avg = np.mean(hist_prices)
                running_std = np.std(hist_prices)
                z_score = (current_price - running_avg) / running_std if running_std > 1e-6 else 0
            
            # Map the z-score to a target duration.
            # A z-score of 0 targets the midpoint. A large negative z-score targets min_duration.
            # A large positive z-score targets max_duration.
            # We use a tanh function to smoothly map z-score to a [-1, 1] range.
            mid_duration = (min_investment_days + max_investment_days) / 2
            duration_range = (max_investment_days - min_investment_days) / 2
            # The z_score is scaled to control sensitivity.
            ...
            actual_investment = min(target_investment, remaining_investment) if remaining_investment > 1e-6 else 0
...
def plot_performance_distribution(results_df: pd.DataFrame):
    fig = px.histogram(results_df, x='performance_bps', nbins=75, histnorm='probability density', title='Distribution of Execution Performance', labels={'performance_bps': 'Performance (bps) vs. Simple Average Price'})
...
def plot_single_path_variable_analysis(price_path: np.ndarray, params: dict):
    ...
    fig.add_trace(go.Scatter(x=days_axis, y=running_simple_avg_price, name="Running Avg. Price", line=dict(color='purple', dash='dash')), row=1, col=1, secondary_y=False)
...
def plot_performance_vs_terminal_price(results_df: pd.DataFrame):
    fig = px.scatter(results_df, x='terminal_price', y='performance_bps', title='Execution Performance vs. Stock Price Outcome', labels={'terminal_price': 'Terminal Stock Price (USD)', 'performance_bps': 'Performance (bps)'}, hover_data=['path_id'], color='performance_bps', color_continuous_scale=px.colors.sequential.Viridis)
...
def run_full_simulation(s0, mu, sigma, n_days, n_paths, strategy_params):
    ...
    if strategy == 'Daily DCA':
        return price_paths, analyze_dca_strategy(price_paths, params_for_analysis['daily_investment'])
...
if 'results_df' in st.session_state and isinstance(st.session_state.results_df, pd.DataFrame):
    ...
    cols = st.columns(5)
    cols[0].metric("Median Performance", f"{perf_bps.median():.1f} bps", help="Positive is better (lower buyback cost).")
    cols[1].metric("Mean Performance", f"{perf_bps.mean():.1f} bps", help="Positive is better.")
    cols[2].metric("Favorable Outcomes", f"{(perf_bps > 0).mean():.1%}", help="Percentage of outcomes where the buyback cost was lower than the simple average price.")
    cols[3].metric("Std. Dev. of Perf.", f"{perf_bps.std():.1f} bps", help="Measures the risk or uncertainty of the execution performance.")
    cols[4].metric("Median Executed USD", f"${results['total_usd_invested'].median():,.0f}", help="The median total capital deployed across all simulations.")
    ...
    with tab1:
        ...
        c1, c2 = st.columns(2)
        c1.plotly_chart(plot_performance_distribution(results), use_container_width=True)
        ...
        if st.session_state.strategy_params['name'] != 'Daily DCA':
            st.plotly_chart(plot_executed_usd_distribution(results), use_container_width=True)
    ...
    with tab5:
        ...
        **4. Volatility-Scaled Adaptive:**
        - **Rule:** This is the most sophisticated strategy. It modulates its daily buyback based on how many standard deviations the current price is away from its running average (its "z-score"). The effect is dampened by the overall market volatility (`sigma`).
        - **Goal:** To be highly aggressive in buying dips during calm markets but cautious and less reactive during periods of high market-wide volatility. It tries to distinguish between a genuine buying opportunity and a random spike in a chaotic market.
        ...
        st.markdown(r"""
        The key performance metric used throughout this application is **Execution Performance in Basis Points (bps)**. It measures how well the chosen strategy timed its repurchases compared to a naive benchmark. It is calculated as:

        $$\text{Performance (bps)} = \left( 1 - \frac{\text{Average Buyback Price}}{\text{Simple Average Stock Price}} \right) \times 10,000$$
        ...
        - **Simple Average Stock Price:** The simple arithmetic mean of the stock's price over the *actual* buyback period for that specific path.
        
        **Interpretation:**
        - **Positive bps:** The strategy was successful. The average buyback price was lower than the simple average stock price, resulting in cost savings for the company.
        - **Negative bps:** The strategy was unsuccessful. The average buyback price was higher than the simple average stock price, meaning the company could have achieved a better price by buying at the average.
        """)
